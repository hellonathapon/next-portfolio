...
function Project({ title, desc, imgPath, stack, urls }: Props) {
  return (
    <div className="mb-10">
      ...
      <article className="p-5">
        ...
        <p className="mt-3 text-sm md:text-md tracking-wider">{desc}</p>
        <div className="flex gap-2 flex-wrap mt-3">
          {stack.map((item, i) => {
            let bg = ParseColor(item);
            return (
              <p
                key={i}
                className={`p-2 text-white rounded-md ${ParseColor(item)}`}
              >
                {item}
              </p>
            );
